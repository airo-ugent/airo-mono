...
import numpy as np
from airo_camera_toolkit.cameras.multiprocess.old_multiprocess_rgb_camera import (
    FrameMetadata,
...
class MultiprocessRGBDPublisher(MultiprocessRGBPublisher):
    ...
    def _setup_framebuffer_writer(self) -> None:
        # Create the shared memory writer for the framebuffer
...
class MultiprocessRGBDReceiver(MultiprocessRGBReceiver, RGBDCamera):
    ...
    def _setup_framebuffer_reader(self, resolution: CameraResolutionType) -> None:
        # Create the shared memory reader for the framebuffer
