...
class MultiprocessRGBDPublisher(MultiprocessRGBPublisher):
    ...
    def _setup(self) -> None:
        ...
        assert isinstance(self._camera, RGBDCamera)  # Check whether user passed a valid camera class
        logger.info(f"Writing resolution information to {self._shared_memory_namespace}_resolution")
    ...
    def run(self) -> None:
        ...
        while not self.shutdown_event.is_set():
            ...
            self._pcd_buf[0, : point_cloud.colors.shape[0]] = point_cloud.colors
...
class MultiprocessRGBDReceiver(MultiprocessRGBReceiver, RGBDCamera):
    ...
    def __init__(self, shared_memory_namespace: str) -> None:
        ...
        # Domain participant and resolution are already initialised in the MultiprocessRGBReceiver superclass.
...
if __name__ == "__main__":
    ...
    publisher = MultiprocessRGBDPublisher(
        Realsense,
        camera_kwargs={
            "resolution": Realsense.RESOLUTION_1080,
            "fps": camera_fps,
        },
    )
