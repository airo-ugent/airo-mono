...
### Changed
- `airo-robots`: `RealmanControl.get_wrench` now uses `airo_typing.WrenchType` instead of a locally-redeclared alias.
...
### Fixed
- CI: the mypy/pytest venv cache was keyed only on `**/setup.py`, so a change to a package's own source files (without touching its `setup.py`) let CI silently type-check/test against a stale installed copy of sibling packages. Broadened the cache key to also hash `**/*.py`. Confirmed: before the fix, `Run MyPy (airo-camera-toolkit)` failed with spurious `"PositionManipulator" has no attribute "start_freedrive"/"stop_freedrive"` errors even though those methods existed in the pushed `airo-robots` source; after the fix, the cache was correctly invalidated and only a real, unrelated type error remained (fixed separately in `cli.py`).
- `airo-camera-toolkit`: `hand_eye_calibration`'s `robot` variable had no type annotation, so mypy inferred its type from the first branch's assignment (`URrtde`), then flagged the `realman` branch's `RealmanControl` assignment as incompatible. Annotated it as `Optional[PositionManipulator]`.
