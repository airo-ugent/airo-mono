...
- `prepare-release.yaml`: The `Create version-bump PR` step no longer fails with a non-fast-forward push error when a `release/<version>` branch from a previous, abandoned run still exists on the remote. When no PR is currently open for that branch (e.g. an old one was closed without merging), the branch is now force-pushed and recreated; when a PR is already open, the branch is left untouched so in-progress manual edits are never clobbered.
