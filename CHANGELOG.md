...
### Fixed
- `airo-camera-toolkit`: the multiprocess camera tests now share one publisher process per module. Starting a second publisher process in a pytest process was reliably undiscovered by receivers on the GitHub CI runners (the publisher reported no matching subscriber and the receiver timed out), although it works locally. The underlying discovery problem is not yet understood.
- `airo-camera-toolkit`: a multiprocess camera receiver whose construction failed (e.g. because the publisher timed out) no longer leaks its Zenoh session. The leaked session kept scouting and stayed a peer of every publisher started later in the same process, so subsequent receivers never received a frame and the interpreter hung on exit.
