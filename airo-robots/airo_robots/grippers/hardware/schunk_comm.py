...
class Schunk40MB(ParallelPositionGripper):
    """
    ...
    def __init__(self, host_ip: str, port: int = 48777, fingers_max_stroke: Optional[float] = None) -> None:
        ...
        see manual p42
        """
        self._gripper_specs = self.SCHUNK_40_MB_DEFAULT_SPECS
        if fingers_max_stroke:
            self._gripper_specs.max_width = fingers_max_stroke
        self.EGUEGK_rpc_ip = host_ip
        self.port = port
    ...
    def connect(self, socket_timeout: float = 2.0) -> None:
        ...
        try:
            self.interpreter_socket.connect((self.EGUEGK_rpc_ip, self.UR_INTERPRETER_SOCKET))
            self.interpreter_socket.settimeout(socket_timeout)
            ...
            raise exc
        try:
            self.common_socket_port_command("interpreter_mode()")
    ...
    def disconnect(self) -> None:
        ...
        self.common_socket_port_command("end_interpreter()")
        self.socket.close()
        self.enable_interpreter_socket.close()

    def clear(self):
        self.common_socket_port_command("clear_interpreter()")
